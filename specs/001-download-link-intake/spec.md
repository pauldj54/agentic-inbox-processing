...
- What happens when the downloaded file is very large (e.g., > 100 MB)? The system should enforce a reasonable size limit per download and skip files that exceed it, logging the skip reason.
...
- **FR-005**: The Cosmos DB email record MUST store attachments in `attachmentPaths` as an array of objects, each containing at minimum `path` (string, blob path) and `source` (`"attachment"` for traditional, `"link"` for link-sourced). `hasAttachments` and `attachmentsCount` MUST reflect all attachments (regular + link-sourced).
...
- **FR-011**: The `source` field on each entry in `attachmentPaths` MUST be set to `"link"` for link-sourced documents and `"attachment"` for traditional email attachments. This is the canonical mechanism for distinguishing attachment origins.
- **FR-012**: The web dashboard SHOULD display the attachment source (link vs. traditional) when showing email details.
...
- "Document" types of interest are primarily PDF, DOCX, XLSX, and CSV. Other content types downloaded from links may be skipped or stored as-is depending on implementation defaults.
- The existing Logic App flow may be extended to handle link detection and download, OR a Python-based pre-processing step may handle it. The implementation plan will determine the best approach.
