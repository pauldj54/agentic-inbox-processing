...
- What happens when the same file appears in the SFTP folder again (duplicate detection)? The system MUST use a two-layer dedup approach: path-based Cosmos DB point-read for initial detection, then content hash (blob MD5) comparison for distinguishing true duplicates from content updates. If a **true duplicate** is detected (same path, same content hash), the delivery is tracked (`deliveryCount` incremented) and downstream processing is skipped. If a **content update** is detected (same path, different content hash), the document `version` is incremented and the file is re-processed through downstream handling (SharePoint re-upload or re-classification).
...
- **FR-005**: Excel and CSV files MUST bypass classification and be uploaded to a SharePoint document library after download, filename metadata parsing, blob storage backup, dedup check, and Cosmos DB logging — without any content extraction or classification step. The `archival-pending` Service Bus queue is NOT used for SFTP-sourced CSV/Excel files.
...
- **FR-009**: All SFTP-sourced files MUST have metadata logged in the Cosmos DB `intake-records` container (renamed from `emails`) using a unified schema. Records are distinguished by the `intakeSource` field (`"email"` or `"sftp"`). Shared base fields (`id`, `partitionKey`, `intakeSource`, `status`, `receivedAt`, `processedAt`, `classification`, `relevanceCheck`, `pipelineMode`, `stepsExecuted`, `queue`, `blobPath`, `createdAt`, `updatedAt`) MUST be present on all records regardless of source. Channel-specific fields coexist without conflict. Existing email records MUST be migrated to the new container, and all code references (Logic App workflow, agent tools, dashboard queries) MUST be updated.
...
- **FR-013**: The system MUST implement 3-way duplicate detection using file path point-read and content hash comparison: (1) **new file** — no existing Cosmos record found, create record and continue downstream processing; (2) **true duplicate** — existing record found with matching content hash, increment `deliveryCount` and skip downstream processing; (3) **content update** — existing record found with different content hash, increment `version` and re-process downstream (re-upload to SharePoint or re-queue for classification).
...
- **FR-018**: Excel and CSV files from SFTP MUST be uploaded to SharePoint Online using HTTP actions with the Microsoft Graph API (`PUT /drives/{driveId}/root:/{path}:/content` with `ActiveDirectoryOAuth` authentication). The SharePoint managed connector is not used because it does not support service principal authentication in the Logic App Consumption tier. The target folder path MUST follow the convention: `{configurable root}/{first letter of Account}/{Account}/{Fund}/{filename}`. Folders are created automatically by Graph API on PUT if they do not exist. The SharePoint drive ID, client ID, client secret, and tenant ID MUST be configurable via Logic App parameters.
...
- **FR-020**: The system MUST track delivery history for SFTP files. Each Cosmos DB record MUST include `version` (starting at 1, incremented on content updates), `deliveryCount` (total deliveries including duplicates), `deliveryHistory` (array of `{deliveredAt, contentHash, action}` entries), and `lastDeliveredAt` (timestamp of most recent delivery).
- **FR-021**: Duplicate detection MUST use `base64(sftpPath)` as the Cosmos DB document `id`, enabling O(1) point-reads for dedup checks. The blob upload MUST occur before the dedup check so the MD5 content hash (`ContentMD5` from blob upload response) is available for comparison.
...
- **CAR-004 (Dependencies)**: The SFTP-SSH connector is a built-in Logic App managed connector. SharePoint uploads use HTTP actions with Graph API (the SharePoint managed connector does not support service principal auth in Consumption tier) — no new code dependencies required. SharePoint Online is a new service dependency for archival of CSV/Excel files. Graph API access requires an Entra ID app registration with `Sites.ReadWrite.All` application permission and a client secret stored in Key Vault.
- **CAR-005 (Auth)**: SFTP authentication MUST use SSH private key auth with the key stored in Azure Key Vault and injected into the API Connection resource at Bicep deployment time. Azure service access (Storage, Cosmos DB, Service Bus) MUST continue using Entra ID managed identity. **Constitution exception (Principle IV)**: SharePoint Graph API authentication uses a client secret (static credential) because the Logic App Consumption tier does not support managed identity for service principal-based Graph API calls. Compensating controls: the client secret is stored in Azure Key Vault with rotation policy, retrieved only at Bicep deployment time via `getSecret()`, and scoped to `Sites.ReadWrite.All` on the target site.
...
- **Intake Record** (Cosmos DB `intake-records` container — unified schema, renamed from `emails`): Both email and SFTP records share the same container, distinguished by `intakeSource` (`"email"` or `"sftp"`). Partition key is `/partitionKey` — a composite value combining the source identifier with the year-month: for emails, `{sender_domain}_{YYYY-MM}` (e.g., `partner-pe-firm.com_2026-03`); for SFTP, `{sftp_username}_{YYYY-MM}` (e.g., `sftp-partner-A_2026-03`). Document `id` for SFTP records is `base64(sftpPath)` (doubles as dedup key for O(1) point-reads); for email records, `id` is the Graph API message ID. Shared base fields present on all records: `id`, `partitionKey`, `intakeSource`, `status` (`"received"`, `"processing"`, `"classified"`, `"archived"`, `"discarded"`, `"needs_review"`, `"error"`), `receivedAt`, `processedAt`, `classification`, `relevanceCheck`, `pipelineMode`, `stepsExecuted`, `queue`, `blobPath`, `createdAt`, `updatedAt`. Email-specific fields (present only when `intakeSource: "email"`): `emailId`, `from`, `subject`, `emailBody`, `hasAttachments`, `attachmentsCount`, `attachmentPaths`. SFTP-specific fields (present only when `intakeSource: "sftp"`): `originalFilename`, `fileType` (`"csv"`, `"xlsx"`, `"xls"`, `"pdf"`), `fileSize`, `sftpPath`, `contentHash` (MD5 from blob upload), `version` (starts at 1, incremented on content updates), `deliveryCount` (total deliveries including duplicates), `deliveryHistory` (array of `{deliveredAt, contentHash, action}` entries), `lastDeliveredAt` (most recent delivery timestamp), `account` (PE fund company, parsed from filename), `fund` (PE fund name, parsed from filename), `docType` (document type, parsed from filename), `publishedDate` (ISO 8601, parsed from filename), `effectiveDate` (ISO 8601, parsed from filename), `docName` (document name, parsed from filename), `metadataParseError` (error details if filename parsing fails, null on success), `sharepointPath` (SharePoint destination path, populated for CSV/Excel files after upload).
...
- The SharePoint site and document library are pre-provisioned. An Entra ID app registration with `Sites.ReadWrite.All` application permission is created by the infrastructure/security team before deployment. The app's client secret is stored in Azure Key Vault. The client secret is injected into the Logic App `sharepointClientSecret` workflow parameter at Bicep deployment time via `getSecret()`.
...
- The Cosmos DB container is renamed from `emails` to `intake-records` to reflect its multi-source nature. Existing email data must be migrated to the new container. All code references (`CONTAINER_EMAILS` in agent tools, Logic App Cosmos DB connector path, dashboard queries) must be updated. The partition key is `/partitionKey` — a composite of source identifier and year-month (e.g., `partner-pe-firm.com_2026-03` for emails, `sftp-partner-A_2026-03` for SFTP), providing natural tenant-per-month data distribution. Existing email records MUST be backfilled with `intakeSource: "email"` and a computed `partitionKey` (derived from sender domain + `receivedAt` month) during migration.
